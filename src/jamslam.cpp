...
const bool JamSlam::isDebug() const {
    return debug;
}

const bool JamSlam::isRunning() const {
    return state != END;
}

const FruitDisplayResult JamSlam::DisplayFruitResult(const FruitResult& result) const {
...
const FruitDisplayResults JamSlam::DisplayFruitResults(const FruitResults& results) const {
...
const BucketDisplayResult JamSlam::DisplayBucketResult(const FruitResults& results) const {
    bool isCatch = false;
    bool isSpike = false;
    bool isBounce = false;
    Color color = WHITE;
...
const int JamSlam::GetScore(const FruitResults& results) const {
    int score = 0;
    for(const FruitResult& result : results){
        score += result.score;
    }
...
const int JamSlam::GetLives(const FruitResults& results) const {
    int lives = 0;
    for(const FruitResult& result : results){
        lives += result.lives;
    }
...
void JamSlam::Update() {
    ...
    if(state == PLAY) {
        ...
        const std::vector<Vector2> projectilesList = bucket.GetProjectiles();
        const FruitResults results = fruits.Update(bucketCollision, projectilesList);
        ...
        const int projectiles = bucket.GetProjectileNumber();
        // Update UI and HUD effects
        const DisplayStats stats = { lives, score, timeLeft, currentLevel, projectiles };
...
void JamSlam::RenderDebug() const {
    if(state != END){
        stage.Render();
        fruits.RenderDebug();
        bucket.RenderDebug();
